...
func TestQrngCache(t *testing.T) {
	...
	if next != uint8(int8cache.data[0]) {
		t.Errorf("initial value %v does not equal first cache value %v", next, int8cache.data[0])
	}
	currentCache := int8cache.data
